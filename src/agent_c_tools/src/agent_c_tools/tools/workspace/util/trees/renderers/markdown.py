...
class MarkdownTreeRenderer(TreeRenderer):
    """
    Renderer that outputs a directory tree in Markdown format with unordered lists.

    This renderer provides a clean, indented list view suitable for Markdown documents.
    """
    ...
    def render(self,
               ...
               **kwargs) -> List[str]:
        """
        Render a DirectoryTree into Markdown format with unordered lists.
    ...
    def _render_node_children(self,
        ...
        Returns:
            List of strings representing the Markdown list structure
        """
        ...
        for child in visible_children:
            ...
            # Add this node to the result with markdown list format
            result.append(f"{prefix}- {display_name}")

            # Only recurse for directories
            if child.is_dir:
                # Calculate next level's prefix (2 space indent for markdown lists)
                next_prefix = prefix + "  "
